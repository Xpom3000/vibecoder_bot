...
def can_checkout(lines: list[CartLine]) -> bool:
    """Оформить заказ можно только из непустой корзины. Чистая функция —
    не трогает БД и не зависит от aiogram, чтобы граничный случай «оформить
    заказ с пустой корзиной» был тестируем напрямую."""
    return bool(lines)


def can_confirm_payment(order: dict | None) -> bool:
    """Подтвердить оплату (перевести статус в «оплачен») можно только у
    заказа, который клиент уже сам отметил как оплаченный (статус
    «на проверке»). Нельзя подтвердить заказ, который клиент ещё не
    оплачивал, и нельзя подтвердить его повторно.

    Чистая функция без побочных эффектов — ключевое бизнес-правило из
    пункта «Оплата» тестируется без БД и без aiogram.
    """
    return order is not None and order["status"] == "на проверке"
